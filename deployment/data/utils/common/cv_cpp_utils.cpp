    ...
    namespace cvUtil
    {
            ...
            for (int ib = 0; ib < (int)batched_result.size(); ++ib)
            {
                ...
                for (auto &obj : objs)
                {
                    ...
                    cv::rectangle(image, cv::Point(obj.left, obj.top), cv::Point(obj.right, obj.bottom),
                                  cv::Scalar(b, g, r), 2);
                    cout<<obj.class_label<<endl;
                    auto name = classlabels[obj.class_label];
                    //std::vector<std::string> name{"Pistola"};
                    auto caption = cv::format("%s %.2f", name.c_str(), obj.confidence);
                    int width = cv::getTextSize(caption, 0, 0.70, 1, nullptr).width + 5;
                    cv::rectangle(image, cv::Point(obj.left - 3, obj.top - 23),
                                  cv::Point(obj.left + width, obj.top), cv::Scalar(b, g, r), -1);
                    cv::putText(image, caption, cv::Point(obj.left, obj.top - 5), 0, 0.70, cv::Scalar::all(0), 1,
                                16);
                }
                if (mkdirs(save_dir))
                {
        ...
        void draw_one_image_rectangle(cv::Mat &image, BoxArray &result, const std::string &save_dir, const std::vector<std::string> &classlabels)
        {
            ...
            for (auto &obj : result)
            {
                ...
                auto name = classlabels[obj.class_label];
                auto caption = cv::format("%s %.2f", name.c_str(), obj.confidence);
                int width = cv::getTextSize(caption, 0, 0.75, 1, nullptr).width + 10;
                cv::rectangle(image, cv::Point(obj.left - 3, obj.top - 33),
                              cv::Point(obj.left + width, obj.top), cv::Scalar(b, g, r), -1);
                cv::putText(image, caption, cv::Point(obj.left, obj.top - 5), 0, 0.75, cv::Scalar::all(0), 1,
                            16);
            }
            if (mkdirs(save_dir))
            {
        ...
        void draw_one_image_rectangle(cv::Mat &image, BoxArray &result, const std::vector<std::string> &classlabels, float thr)
        {
        ...
        void draw_one_image_rectangle_2(cv::Mat &image, BoxArray &result, const std::vector<std::string> &classlabels, float thr )
        {
            ...
            for (auto &obj : result)
            {
                ...
                cv::rectangle(image, cv::Point(obj.left - 3, obj.top - 33),
                              cv::Point(obj.left + width, obj.top), cv::Scalar(b, g, r), -1);
                cv::putText(image, caption, cv::Point(obj.left, obj.top - 5), 0, 1, cv::Scalar::all(0), 4,
                            16);
                }
            }
        }

        void draw_one_image_rectangle_3(cv::Mat &image, BoxArray &result, const std::vector<std::string> &classlabels, float thr, bool flag)
        {
            ...
            for (auto &obj : result)
            {
                ...
                auto name = classlabels[obj.class_label];
                auto caption = cv::format("%s %.2f", name.c_str(), obj.confidence);
                int width = cv::getTextSize(caption, 0, 0.70, 2, nullptr).width+10;
                cv::rectangle(image, cv::Point(obj.left - 3, obj.top - 21),
                              cv::Point(obj.left + width, obj.top), cv::Scalar(b, g, r), -1);
                cv::putText(image, caption, cv::Point(obj.left, obj.top - 3), 0, 0.70, cv::Scalar::all(0), 2,
                            16);
                }
            }
        }
